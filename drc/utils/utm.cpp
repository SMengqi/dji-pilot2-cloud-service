...
* 参数说明：
* a: MAJOR_AXIS 轴长
* e2: SQUARED_ECCENTRICITY 偏心率
* zone: UTM坐标所在的时区
* N: 传入参数  utm_north: UTM_Y
* E: 传入参数  utm_east : UTM_X
* lat_rad: 传出参数 latitude: 纬度
* lon_rad: 传出参数 longitude: 经度
*/
...
/*  UTM坐标转为WGS84标准的经纬度坐标
 *  utm_east: UTM_X ,  utm_north: UTM_Y
 *  Latitude: 纬度  ,  Longitude: 经度
 *  Zone: 时区 1-60
*/
int UTM2WGS84(double utm_east,double utm_north, double &latitude,double &longitude,int zone)
{
    Hemisphere hemi = HEMI_NORTH; //固定为北半球
    double lat_tmp, lon_tmp;
    std::stringstream stream , stream2;

    if(utm_east <= 0 || utm_north<= 0)
    {
        return -1;
    }
    
    if(zone < UTM_ZONE_1 || zone > UTM_ZONE_60)
    {
        return  -2;
    }

    grid_to_geographic(MAJOR_AXIS, SQUARED_ECCENTRICITY, (GridZone)zone, hemi, utm_north, utm_east, &lat_tmp, &lon_tmp);

    lat_tmp = lat_tmp/M_PI*180.0;
    lon_tmp = lon_tmp/M_PI*180.0;


    stream << std::fixed << std::setprecision(6) << lat_tmp;
    stream >> latitude;

    stream2 << std::fixed << std::setprecision(6) << lon_tmp;
    stream2 >> longitude;
...
/*  将WGS84标准的经纬度坐标转为UTM坐标
 *  utm_east: UTM_X ,  utm_north: UTM_Y
 *  latitude: 纬度  ,  longitude: 经度
 *  zone: 时区
*/
int WGS842UTM(double latitude,double longitude,double &utm_east,double &utm_north,int &zone)
{
    double lat = 32.0/180.0*M_PI;
    double lon = -120/180.0*M_PI;
    GridZone Gzone = GRID_AUTO;
    ...
    geographic_to_grid(MAJOR_AXIS, SQUARED_ECCENTRICITY, lat, lon, &Gzone, &hemi, &utm_north, &utm_east);
