...
#undef _CONVERT
    ...
    if (!jf) 
    {
        pl_log(ERR, "Fail to convert to json");
    }
    return jf;
}
...
#define _CONVERT(type, ctype, fmt, sfunc, afunc)         \
        ...
        case FieldDescriptor::CPPTYPE_ENUM: {
            ...
            {
                pl_log(ERR, "Not an integer or string");
            }

            if (!ev)
            {
                pl_log(ERR, "Enum value not found");
            }
...
static void _json2pb(Message& msg, json_t *root)
{
    ...
    if (!d || !ref) 
    {
        pl_log(ERR, "No descriptor or reflection");
        return;
    }
    ...
    for (void *i = json_object_iter(root); i; i = json_object_iter_next(root, i))
    {
        ...
        if (!field) 
        {
            pl_log(ERR, "Unknown field: %s", name);
            return;
        }
...
bool json2pb(Message &msg, const char *buf, size_t size)
{
    json_t *root;
    json_error_t error;
    ...
    if (!root)
    {
        pl_log(ERR, "Load failed: %s", error.text);
        return false;
    }
    ...
    {
        pl_log(ERR, "Malformed JSON: not an object");
    }
