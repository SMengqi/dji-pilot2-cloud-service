          ...
          struct sockaddr **addrs)
{
    ...
    getaddrs.addrs = (struct sockaddr *)pf_malloc(len);
          ...
          struct sockaddr **addrs)
{
    ...
    struct sctp_getaddrs *getaddrs = (struct sctp_getaddrs*)pf_malloc(bufsize);
    if(!getaddrs)
        return -1;
    ...
    for(;;) {
        ...
        new_buf = pf_realloc(getaddrs, bufsize+4096);
        if (!new_buf) {
            pf_free(getaddrs);
            return -1;
        }
